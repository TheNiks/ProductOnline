...
final class ProductStorage {
    ...
    func getAllProducts() throws -> Products {
        let jsonString = self.loadContent()
    ...
    func getProductDetail(proId: Int) throws -> Product {
        let jsonString = self.loadContent()
        let jsonData = Data(jsonString.utf8)
        let decoder = JSONDecoder()
        do {
            let products = try decoder.decode(Products.self, from: jsonData)
            if let product = products.products.first(where: { $0.id == proId }) {
                return product
            } else {
